function getMaximumGold(grid: number[][]): number {
    ...
    function dfs(r: number, c: number): number {
        ...
        if (gridObjs[r][c].visited || gridObjs[r][c].value == 0) return 0;

        gridObjs[r][c].visited = true;
        const result = gridObjs[r][c].value + Math.max( 
            dfs(r, c-1),
            dfs(r, c+1),
            dfs(r-1, c),
            dfs(r+1, c),
...
function exist(board: string[][], word: string): boolean {
    ...
    function dfs(r: number, c: number, j: number): boolean {
        if (r <0 || r > rows-1 || c < 0 || c > columns-1) return false;
        if (boardObjs[r][c].visited) return false;
...
function canPartitionKSubsets(nums: number[], k: number): boolean {
    ...
    nums.sort((a, b)=> a -b);  
    const s: any = new Set(nums);
    const maxNum: number = Math.max(...nums);
    let dp: number[][] =  [...Array(n+1)].map(row => [...Array(maxNum + 1)].fill(0));
    for(let i=0; i<n+1; i++)
        dp[i][0] = 1;
...
function findDifferentBinaryString(nums: string[]): string {
    ...
    let s: number[] = nums.map(x => parseInt(x, 2));
    let result: string[]= [];
    
    function replace(str: string[], j: number) {
        for(let i=0; i < n; i++ ) {
            if (j == i) {
                str[i] = (str[i] == '0') ? '1': '0';
            }
    ...
    function dfs(str: string[], index: number): void {
...
function solve(board: string[][]): void {
    ...
    let objVals: any[] = board.map(row => row.map(col => {
        let obj: any = {};
        obj["value"] = col;
        obj["visited"] = false;
        return obj;
    ...
    function dfs(r: number, c: number): boolean {   
        ...
        if (objVals[r][c].visited) return true;

        objVals[r][c].visited = true;
        if (objVals[r][c].value == 'X') return true;
        if (objVals[r][c].value == 'O' && (r == 0) || (r == rows-1) || (c == 0) || (c == cols-1)) return false;
        
        let result: boolean = dfs(r-1, c) && dfs(r+1, c) && dfs(r, c-1) && dfs(r, c+1); 
        if (result)
            objVals[r][c].value = 'X';
        return result;
    }
    ...
    board = objVals.map(row => row.map((col: any) => col["value"]));
    //console.log(board);
};
