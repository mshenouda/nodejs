...
function bfs(nums: any[]): TreeNode | null {
    ...
    while (queue.length > 0) {
        let parent: TreeNode | null = queue.shift();
        let j: number = 0;
...
function largestValues2(root: TreeNode | null): number[] {
    function bfs(root: TreeNode): any {
        ...
        while (queue.length > 0) {
            ...
            const newLevel: number = node["level"];
    ...
    const result: TreeNode = bfs(root);
    return []; 
}
// const nums = [-40, 0, -37];
// const root: TreeNode = bfs(nums);
...
function findBottomLeftValue(root: TreeNode | null): number {
    let obj: any = {};
    function dfs(root: TreeNode, height: number): void {
        if (root == null) return;
        if(root) {
    ...
    dfs(root, 0);
    const maxKey = Object.keys(obj).sort((a: any, b: any) => b-a)[0];
    return obj[maxKey];
};
...
function isEvenOddTree(root: TreeNode | null): boolean {
    let obj: any = {};
    function dfs(root: TreeNode | null | null, height: number): void {
        if (root == null) return;
        if(root) {
            if (obj[height] == undefined)
                obj[height] = [root.val];
    ...
    for([k, values] of Object.entries(obj)) {
        if (Number(k) % 2 == 0) {
            if (!isStrictlyIncreasing(values)) return false;
            if(!values.every((x: any) => x % 2 == 1)) return false;
        }
        if (Number(k) % 2 == 1) {
            if (!isStrictlyDecreasing(values)) return false;    
            if(!values.every((x: any) => x % 2 == 0)) return false;
        }
    }
    return true;
};
...
function treeHeight(root:TreeNode): number
{
    ...
    function dfs(root: TreeNode | null| null, height: number) {
        if(root == null) return 0;    
...
function traverseTree(root: TreeNode): any[] {
    if (root == null)
        return [];
...
function bstToGst(root: TreeNode | null): TreeNode | null {
    ...
    dfs(root);
    const vals: number[] = traverseTree(root);
    console.log(vals);
    return root;
};
...
function rob(root: TreeNode | null): number {
    ...
    dfs(root, 0);
    console.log(obj);
    const n: number = Object.keys(obj).length;
    let dp: any[] = [...Object.values(obj)];
...
function balanceBST(root: TreeNode | null): TreeNode | null {
    ...
    //construct tree
    const result: TreeNode | null = bfs(nums);
    return result;
};
...
function getTargetCopy(original: TreeNode | null, cloned: TreeNode | null, target: TreeNode | null): TreeNode | null { 
    ...
    function dfs(original: TreeNode, cloned: TreeNode): void {
...
function removeLeafNodes(root: TreeNode | null, target: number): TreeNode | null {
    ...
    dfs(root);
    const values: number[] = traverseTree(root);
    console.log(values);
    return null;
};
...
function flatten(root: TreeNode | null): void {
    ...
    function dfs(root: TreeNode | null, oldRoot: TreeNode| null = null): void {
        if (root == null) return;
        const tmpRoot: TreeNode | null = new TreeNode(root.val);
        if (oldRoot == null) {
            oldRoot = tmpRoot;
            newRoot = oldRoot;
        } else {
    ...
    dfs(root);
    const result: number[] = traverseTree(newRoot);
    console.log(result);
};
...
function pathSum4(root: TreeNode | null, targetSum: number): number[][] {
    ...
    let total: number[][] = [];
    function dfs(root: TreeNode | null, sum: number, result: number[] = []): void {
        if (root == null) return;
...
function findTargetSumWays(nums: number[], target: number): number {
    ...
    let obj: any = {};
    function dfs(i: number, sum: number = 0): number {
        if (i == n) return (sum == target) ? 1 : 0;
        if (obj[`${i},${sum}`] != undefined) return obj[`${i},${sum}`]; 
...
function maxAncestorDiff(root: TreeNode | null): number {
    if (root == null) return 0;
    let maxVal = 0;
    function dfs(root: TreeNode, parent: number, diff: number) {
        if (root == null) return 0;
...
function countVowels(word: string): number {
    let c = 0
    let n = word.length
    let vowels: any = {'a': 1, 'e': 1, 'o': 1, 'u': 1, 'i': 1}
    let l = 0;
    let r = 0;
    for (let i=0; i<n; i++) {
        if (vowels[word[i]]) {
...
function numberOfSubstrings(s: string): number {
    ...
    function firstIndex(arr: number[], index: number): number {
        for(let i=0; i < arr.length; i++)
            if (arr[i] >= index) return arr[i];
        return -1;
    }
...
function findFrequentTreeSum(root: TreeNode | null): number[] {
    ...
    dfs(root);
    const values: any[] = Object.entries(dp).sort((a: any, b: any) => b[1] - a[1]);
    let maxResult = 0;
    let result: number[] = [];
...
function averageOfSubtree(root: TreeNode | null): number {
    ...
    function dfs(root: TreeNode | null, direction: string): any {
        if (root == null)
            return {count: 0, direction: ""};
        
        let l: any = {count: 0, direction: ""};
        let r: any = {count: 0, direction: ""};
        let result: any = {count: 0, direction: direction};
...
function flattened(root: TreeNode | null): TreeNode | null{
    let queue: any[] = []
...
function widthOfBinaryTree(root: TreeNode | null): number {
    ...
    let obj: myObj = {};
    function dfs(root: TreeNode | null, level: number=0): number {
        if (root == null) return 0;
        let pos: number = 0;
        if (obj[level] == undefined)
        {
...
function mctFromLeafValues(arr: number[]): number {
    let dp: number[] = [...Array(arr.length)].fill(0);
    for(let i=0; i<arr.length; i++) {
...
function reverseOddLevels(root: TreeNode | null): TreeNode | null {
    ...
    function dfs(root: TreeNode | null, level: number = 0): void {
        if (root == null) return;
        let tmpLeft: TreeNode | null = root.left;
        let tmpRight: TreeNode | null = root.right; 
        if (root.left) {
            if (level % 2 == 0) 
                dfs(root.left, level+1);
            else {
