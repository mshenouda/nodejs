...
function findBottomLeftValue(root) {
    ...
    dfs(root, 0);
    var maxKey = Object.keys(obj).sort(function (a, b) { return b - a; })[0];
    return obj[maxKey];
}
;
    ...
    function dfs(root, height) {
        ...
        if (root !== null) {
            ...
            if (found && height - leastHeight == k)
                values = __spreadArray(__spreadArray([], values, true), [root.val], false);
        }
        if (root.left)
            dfs(root.left, height + 1);
        if (root.right)
...
function isEvenOddTree(root) {
    ...
    function dfs(root, height) {
        ...
        if (root) {
            if (obj[height] == undefined)
                ...
                obj[height] = __spreadArray(__spreadArray([], obj[height], true), [root.val], false);
        }
        if (root.left)
            dfs(root.left, height + 1);
        if (root.right)
    ...
    for (var _i = 0, _b = Object.entries(obj); _i < _b.length; _i++) {
        _a = _b[_i], k = _a[0], values = _a[1];
        if (Number(k) % 2 == 0) {
            if (!isStrictlyIncreasing(values))
                return false;
            if (!values.every(function (x) { return x % 2 == 1; }))
                return false;
        }
        if (Number(k) % 2 == 1) {
            if (!isStrictlyDecreasing(values))
                return false;
            if (!values.every(function (x) { return x % 2 == 0; }))
                return false;
        }
    }
    return true;
...
function rob(root) {
    ...
    var dp = __spreadArray([], Object.values(obj), true);
    var maxResult = 0;
    for (var i = 0; i < n; i++) {
...
function balanceBST(root) {
    ...
    function inOrder(root) {
        ...
        if (root.left)
            inOrder(root.left);
        nums = __spreadArray(__spreadArray([], nums, true), [root.val], false);
        if (root.right)
            inOrder(root.right);
    }
    function bfs(nums) {
...
function kthSmallest(root, k) {
    function dfs(root) {
        ...
        var result = [];
        if (root.left)
            result = __spreadArray(__spreadArray([], result, true), dfs(root.left), true);
        result = __spreadArray(__spreadArray([], result, true), [root.val], false);
        if (root.right)
            result = __spreadArray(__spreadArray([], result, true), dfs(root.right), true);
        return result;
    }
    var result = dfs(root);
    return result[k - 1];
    ...
    function bfs(root) {
        ...
        while (queue.length > 0) {
            ...
            if (obj[level] == undefined)
                ...
                obj[level] = __spreadArray(__spreadArray([], obj[level], true), [node["value"].val], false);
            if (node["value"].left) {
...
function flatten(root) {
    var newRoot = null;
    function dfs(root, oldRoot) {
        if (oldRoot === void 0) { oldRoot = null; }
        if (root == null)
            return;
        var tmpRoot = new TreeNode(root.val);
        if (oldRoot == null) {
            oldRoot = tmpRoot;
            newRoot = oldRoot;
        }
    ...
    dfs(root);
    var result = traverseTree(newRoot);
    console.log(result);
}
;
...
function sumNumbers(root) {
    ...
    function dfs(root, level) {
        ...
        if (obj[level] == undefined)
            ...
            obj[level] = __spreadArray(__spreadArray([], obj[level], true), [root.val], false);
        if (root.left)
            dfs(root.left, level + 1);
        counter++;
        if (root.right)
    ...
    var counter = 1;
    for (var i = n - 1; i > -1; i--) {
        var multiplier = Math.pow(10, (n - 1 - i));
        var partial = 0;
        var j = 0;
        for (var _i = 0, _a = values[i]; _i < _a.length; _i++) {
            value = _a[_i];
            partial += value;
            j++;
        }
...
function pathSum4(root, targetSum) {
    ...
    function dfs(root, sum, result) {
        ...
        if (root.left == null && root.right == null && sum == root.val) {
            var tmp = __spreadArray([], result, true);
            total = __spreadArray(__spreadArray([], total, true), [tmp], false);
        }
        if (root.left)
...
function findTargetSumWays(nums, target) {
    ...
    function dfs(i, sum) {
        ...
        if (obj["".concat(i, ",").concat(sum)] != undefined)
            return obj["".concat(i, ",").concat(sum)];
        obj["".concat(i, ",").concat(sum)] = dfs(i + 1, sum + nums[i]) + dfs(i + 1, sum - nums[i]);
        return obj["".concat(i, ",").concat(sum)];
    }
    var result = dfs(0);
    console.log(obj);
...
function countVowels(word) {
    var c = 0;
    var n = word.length;
    var vowels = { 'a': 1, 'e': 1, 'o': 1, 'u': 1, 'i': 1 };
    var l = 0;
    var r = 0;
    for (var i = 0; i < n; i++) {
        if (vowels[word[i]]) {
...
function numberOfSubstrings(s) {
    ...
    var result = 0;
    for (var v = 0; v < n; v++) {
        var i = firstIndex(a, v);
        var j = firstIndex(b, v);
        var k = firstIndex(c, v);
        if (i > -1 && j > -1 && k > -1) {
            var index = Math.max(i, j, k);
            result += (n - index);
        }
    }
    console.log(a, b, c);
...
function findFrequentTreeSum(root) {
    ...
    dfs(root);
    var values = Object.entries(dp).sort(function (a, b) { return b[1] - a[1]; });
    var maxResult = 0;
    var result = [];
    for (var _i = 0, values_1 = values; _i < values_1.length; _i++) {
        ...
        if (value[1] >= maxResult) {
            maxResult = value[1];
            result = __spreadArray(__spreadArray([], result, true), [value[0]], false);
        }
    }
    return result;
}
...
function averageOfSubtree(root) {
    ...
    function dfs(root, direction) {
        ...
        var l = { count: 0, direction: "" };
        var r = { count: 0, direction: "" };
        var result = { count: 0, direction: direction };
        if (root.left)
            l = dfs(root.left, "l");
        if (root.right)
            r = dfs(root.right, "r");
...
function rightSideView(root) {
    ...
    for (var _i = 0, _a = Object.values(obj); _i < _a.length; _i++) {
        var val = _a[_i];
        result = __spreadArray(__spreadArray([], result, true), [val[val.length - 1]], false);
    }
    return result;
}
;
...
function mctFromLeafValues(arr) {
    var dp = __spreadArray([], Array(arr.length), true).fill(0);
    for (var i = 0; i < arr.length; i++) {
        var maxNum = -Infinity;
